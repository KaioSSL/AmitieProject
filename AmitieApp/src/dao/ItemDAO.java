        ...
        try {
            if (item.getCod_tipo() == 1) {
                ...
                stmt = con.prepareStatement(
                        "INSERT INTO ITEM_SAIDA(TIPO,VLR_UNIT,QTD,VLR_TOTAL,UNIDADE_TOTAL,DAT_CAD,COD_ARMAZEM,COD_PEDIDO,COD_PRODUTO) VALUES(?,?,?,?,?,CURRENT_DATE,?,?,?)");
                ...
                // Escrevo minha query utilizando ? para posicionar os parametros
                stmt = con.prepareStatement(
                        "INSERT INTO ITEM_ENTRADA(TIPO,VLR_UNIT,VLR_TOTAL,UNIDADE_TOTAL,DAT_CAD,COD_ARMAZEM,COD_PEDIDO,COD_MATERIA_PRIMA) VALUES(?,?,?,?,?,CURRENT_DATE,?,?,?)");
                // Insiro os parametros em cada posição e com seu tipo
    ...
    public static boolean update(Item item) {
        ...
        try {
            if (item.getTipo() == 1) {
                ...
                // Escrevo minha query utilizando ? para posicionar os parametros
                stmt = con.prepareStatement(
                        "UPDATE ITEM_SAIDA SET TIPO = ?, VLR_UNIT = ?, QTD = ?, VLR_TOTAL = ?, UNIDADE_TOTAL = ?, COD_ARMAZEM = ?, COD_PEDIDO = ?, COD_PRODUTO = ? WHERE COD = ?");
                // Insiro os parametros em cada posição e com seu tipo
                stmt.setInt(1, item_saida.getCod_tipo());
                stmt.setFloat(2, item_saida.getVlr_unit());
                stmt.setFloat(3, item_saida.getQtd());
                ...
                // Escrevo minha query utilizando ? para posicionar os parametros
                stmt = con.prepareStatement(
                        "UPDATE ITEM_ENTRADA SET TIPO = ?, VLR_UNIT = ?, QTD = ?, VLR_TOTAL = ?, UNIDADE_TOTAL = ?, COD_ARMAZEM = ?, COD_PEDIDO = ?, COD_MATERIA_PRIMA = ? WHERE COD = ?");
                // Insiro os parametros em cada posição e com seu tipo
                stmt.setInt(1, item_entrada.getCod_tipo());
                stmt.setFloat(2, item_entrada.getVlr_unit());
                stmt.setFloat(3, item_entrada.getQtd());
